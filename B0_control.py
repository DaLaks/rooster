...
class Control:
        ...
        for line in lines:
            ...
            if len(word) > 0:
                ...
                # cladding
                    ...
                    inp['clad'].append(
                        {'id': word[1], 'matid': word[2], 'ri': word[3], 'ro': word[4], 'nr': int(word[5])})
                # --------------------------------------------------------------------------------------
                # core geometry
                elif key == 'coregeom':
                    if len(word) - 1 < 4:
                        print(
                            '****ERROR: \'coregeom\' card should have four values after the keyword: geometry flag (hex01, hex06, hex24, square), pitch (distance between node centres), bottom boundary conditions (0: vacuum, -1: reflective), top boundary conditions (0: vacuum, -1: reflective).')
                        sys.exit()
                    ...
                    if word[3] != 0 and word[3] != 1:
                        print('****ERROR: bottom boundary condition flag of \'coregeom\' card (word 4) is wrong: ',
                              word[3], '\nCorrect values are:\n0 (vacuum)\n1 (reflective)')
                        sys.exit()
                    if word[4] != 0 and word[4] != 1:
                        print('****ERROR: top boundary condition flag of \'coregeom\' card (word 5) is wrong: ',
                              word[4], '\nCorrect values are:\n0 (vacuum)\n1 (reflective)')
                        sys.exit()
                ...
                # fuel rod card
                    ...
                    if any([id in x['id'] for x in inp['fuelrod']]):
                        ...
                        inp['fuelrod'].append(
                            {'id': id, 'fuelid': [word[2]], 'hgap': [float(word[3])], 'cladid': [word[4]],
                             'p2d': [word[5]], 'mltpl': [word[6]], 'pipeid': [word[7]], 'pipenode': [int(word[8])]})\

                            # 'kr': [int(word[9])], 'kz': [int(word[10])]})
                # --------------------------------------------------------------------------------------
                # heat structure card
                elif key == 'htstr':
                    inp['htstr'].append({'id': word[1], 'matid': word[2], 'ri': float(word[3]), 'ro': float(word[4]),
                                         'nr': int(word[5]), 'bcleft': word[6], 'bcright': word[7], 'mltpl': word[8]})
                # --------------------------------------------------------------------------------------
                # inner gas
                elif key == 'innergas':
                ...
                # material
                    ...
                    if word[2] == 'he':
                        ...
                        inp['mat'].append(
                            {'id': word[1], 'type': word[2], 'pu': word[3], 'b': word[4], 'x': word[5], 'por': word[6],
                             'temp0': word[7]})
                    elif word[2] == 'na':
                        inp['mat'].append({'id': word[1], 'type': word[2], 'p0': word[3], 'temp0': word[4]})
                    elif word[2] == 'ss316':
                ...
                # mixture of isotopes
                    ...
                    if len(word) - 1 < 4:
                        print(
                            '****ERROR: mix card should have four values after the keyword: mix id, isotopeid, number density and signal id for temperature.')
                        sys.exit()
                    ...
                    if any([mixid in x['mixid'] for x in inp['mix']]):
                        ...
                        inp['mix'].append(
                            {'mixid': mixid, 'isoid': [word[2]], 'numdens': [float(word[3])], 'signaltemp': [word[4]]})
                # --------------------------------------------------------------------------------------
                ...
                # thermal-hydraulic pipe without free level
                    ...
                    inp['pipe'].append(
                        {'id': word[1], 'type': 'normal', 'matid': word[2], 'dhyd': word[3], 'len': word[4],
                         'dir': word[5], 'areaz': word[6], 'nnodes': int(word[7]), 'signaltemp': ''})
                # --------------------------------------------------------------------------------------
                # thermal-hydraulic pipe with free level
                elif key == 'pipe-f':
                    inp['pipe'].append(
                        {'id': word[1], 'type': 'freelevel', 'matid': word[2], 'dhyd': word[3], 'len': word[4],
                         'dir': word[5], 'areaz': word[6], 'nnodes': 1, 'signaltemp': ''})
                # --------------------------------------------------------------------------------------
                # thermal-hydraulic pipe without free level with temperature defined by signal
                elif key == 'pipe-t':
                    inp['pipe'].append(
                        {'id': word[1], 'type': 'normal', 'matid': word[2], 'dhyd': word[3], 'len': word[4],
                         'dir': word[5], 'areaz': word[6], 'nnodes': int(word[7]), 'signaltemp': word[8]})
                # --------------------------------------------------------------------------------------
                ...
                # models to be solved
                    ...
                    if word[1] == 'spatialkinetics':
                        ...
                        try:
                            ...
                            print(
                                '****ERROR: the second value after the keyword of solve spatialkinetics card should be integer (number of energy groups), e.g.:\nsolve spatialkinetics 25')
                            sys.exit()
                        # check that the third value is DIF, MC or AD
                        if word[3] != 'DIF' and word[3] != 'MC' and word[3] != 'AD':
                            print('****ERROR: solve spatialkinetics card should have the third value of method indicator either DIF (neutron diffusion solver) or AD (adjoint neutron diffusion solver) or MC (Monte Carlo method)')
                            sys.exit()
                        # method indicator
                        inp['nmeth'] = word[3]
                # --------------------------------------------------------------------------------------
                # stack of mixes of isotopes
                    ...
                    if len(word) - 1 < 4:
                        print(
                            '****ERROR: stack card should have four values after the keyword: stack id, mix id, pipe id, pipe node.')
                        sys.exit()
                    ...
                    if any([stackid in x['stackid'] for x in inp['stack']]):
                        ...
                        inp['stack'].append(
                            {'stackid': stackid, 'mixid': [word[2]], 'pipeid': [word[3]], 'pipenode': [int(word[4])]})
                # --------------------------------------------------------------------------------------
                ...
                # thermal boundary conditions]
                    ...
                    try:
                        ...
                        print('****ERROR: boundary condition type of thermbc card (word 3) is wrong: ', word[2],
                              '. Correct values are: 0 (heat flux), 1 (heat exchange coefficient and temperature) or 2 (pipe id and pipenodeid).')
                        sys.exit()
                    if dict['type'] == 0:
                        if len(word) - 1 < 3:
                            print(
                                '****ERROR: thermbc card with type == 0 should have three values after the keyword: id, type and qf.')
                            sys.exit()
                        dict['qf'] = word[3]
                    elif dict['type'] == 1:
                        if len(word) - 1 < 4:
                            print(
                                '****ERROR: thermbc card with type == 1 should have four values after the keyword: id, type, alfa and temp.')
                            sys.exit()
                        dict['alfa'] = word[3]
                        dict['temp'] = word[4]
                    elif dict['type'] == 2:
                        if len(word) - 1 < 4:
                            print(
                                '****ERROR: thermbc card with type == 2 should have four values after the keyword: id, type, pipeid and pipenode.')
                            sys.exit()
                        dict['pipeid'] = word[3]
    ...
    def open_output_files(self, reactor):
        ...
        if 'signal' in self.input:
            ...
            fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join(
                [(self.input['signal'][j]['id']).ljust(13) for j in range(len(self.input['signal']))] + [
                    table['f(x)'][0].ljust(13) for table in self.input['lookup']]) + '\n')
        if 'fluid' in reactor.solve:
            fid.append(open(path4results + os.sep + 'fluid-mdot.dat', 'w'))
            fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join(
                [(self.input['junction']['from'][j] + '-' + self.input['junction']['to'][j]).ljust(13) for j in
                 range(reactor.fluid.njuni + reactor.fluid.njund)]) + '\n')
            for i in range(reactor.fluid.npipe):
                fid.append(open(path4results + os.sep + 'fluid-p-' + reactor.fluid.pipeid[i] + '.dat', 'w'))
                fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join(
                    [str(j).zfill(4).ljust(13) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
        ...
        if 'fuelrod' in reactor.solve:
            for i in range(reactor.solid.nfuelrods):
                fid.append(
                    open(path4results + os.sep + 'fuelrod-hgap-' + [x['id'] for x in self.input['fuelrod']][i] + '.dat',
                         'w'))
                fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join(
                    [('hgap-' + str(j).zfill(3)).ljust(13) for j in range(reactor.solid.fuelrod[i].nz)]) + '\n')
                for j in range(reactor.solid.fuelrod[i].nz):
                    fid.append(open(path4results + os.sep + 'fuelrod-temp-' + [x['id'] for x in self.input['fuelrod']][
                        i] + '-' + str(j + 1).zfill(3) + '.dat', 'w'))
                    fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join(
                        [('tempf-' + str(k).zfill(3) + '(K)').ljust(13) for k in
                         range(reactor.solid.fuelrod[i].fuel[j].nr)]) + ''.join(
                        [('tempc-' + str(k).zfill(3) + '(K)').ljust(13) for k in
                         range(reactor.solid.fuelrod[i].clad[j].nr)]) + '\n')
        ...
        if 'htstr' in reactor.solve:
            for i in range(reactor.solid.nhtstr):
                fid.append(
                    open(path4results + os.sep + 'htstr-temp-' + [x['id'] for x in self.input['htstr']][i] + '.dat',
                         'w'))
                fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join(
                    [('temp-' + str(j).zfill(3)).ljust(13) for j in range(reactor.solid.htstr[i].nr)]) + '\n')
        if 'pointkinetics' in reactor.solve:
            ...
            fid[-1].write(' ' + 'time(s)'.ljust(13) + ''.join(
                [('cdnp-' + str(i)).ljust(13) for i in range(reactor.core.ndnp)]) + '\n')
        if 'spatialkinetics' in reactor.solve:
            ...
            fid[-1].write(' ' + 'time(s)'.ljust(13) + 'igroup'.ljust(13) + 'iz'.ljust(13) + 'ix'.ljust(13) + 'iy'.ljust(
                13) + 'flux'.ljust(13) + '\n')
            if (reactor.core.meth == 'AD'):
                fid.append(open(path4results + os.sep + 'core-adjoint-flux.dat', 'w'))
                fid[-1].write(
                    ' ' + 'time(s)'.ljust(13) + 'igroup'.ljust(13) + 'iz'.ljust(13) + 'ix'.ljust(13) + 'iy'.ljust(
                        13) + 'adjoint flux'.ljust(13) + '\n')
            fid.append(open(path4results + os.sep + 'core-pow.dat', 'w'))
            fid[-1].write(
                ' ' + 'time(s)'.ljust(13) + 'iz'.ljust(13) + 'ix'.ljust(13) + 'iy'.ljust(13) + 'pow'.ljust(13) + '\n')
    ...
    def print_output_files(self, reactor, fid, time, flag):
        ...
        if 'fluid' in reactor.solve:
            ...
            indx += 1
            for i in range(reactor.fluid.npipe):
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                    ['{0:12.5e} '.format(reactor.fluid.p[i][j]) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                indx += 1
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                    ['{0:12.5e} '.format(reactor.fluid.temp[i][j]) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                indx += 1
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                    ['{0:12.5e} '.format(reactor.fluid.vel[i][j]) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                indx += 1
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                    ['{0:12.5e} '.format(reactor.fluid.re[i][j]) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                indx += 1
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                    ['{0:12.5e} '.format(reactor.fluid.pr[i][j]) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                indx += 1
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                    ['{0:12.5e} '.format(reactor.fluid.pe[i][j]) for j in range(reactor.fluid.pipennodes[i])]) + '\n')
                indx += 1
        if 'fuelrod' in reactor.solve:
            for i in range(reactor.solid.nfuelrods):
                # gas gap conductance
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                    ['{0:12.5e} '.format(reactor.solid.fuelrod[i].innergas.hgap[j]) for j in
                     range(reactor.solid.fuelrod[i].nz)]) + '\n')
                indx += 1
                # fuel and clad temperatures
                for j in range(reactor.solid.fuelrod[i].nz):
                    fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                        ['{0:12.5e} '.format(reactor.solid.fuelrod[i].fuel[j].temp[k]) for k in
                         range(reactor.solid.fuelrod[i].fuel[j].nr)]) + ''.join(
                        ['{0:12.5e} '.format(reactor.solid.fuelrod[i].clad[j].temp[k]) for k in
                         range(reactor.solid.fuelrod[i].clad[j].nr)]) + '\n')
                    indx += 1
                    for k in range(reactor.solid.fuelrod[i].fuel[j].nr):
                        if 'fuelgrain' in reactor.solve and i + j + k == 0:
                            fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                                ['{0:12.5e} '.format(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].c1[l]) for l in
                                 range(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].nr)]) + '\n')
                            indx += 1
                            fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                                ['{0:12.5e} '.format(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].ri[l]) for l in
                                 range(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].NB)]) + '\n')
                            indx += 1
                            fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                                ['{0:12.5e} '.format(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].cv_irr[l]) for l in
                                 range(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].NB)]) + '\n')
                            indx += 1
                            fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                                ['{0:12.5e} '.format(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].ci_irr[l]) for l in
                                 range(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].NB)]) + '\n')
                            indx += 1
                            fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                                ['{0:12.5e} '.format(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].cv_p[l]) for l in
                                 range(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].NB)]) + '\n')
                            indx += 1
                            fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                                ['{0:12.5e} '.format(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].bi[l]) for l in
                                 range(reactor.solid.fuelrod[i].fuel[j].fuelgrain[k].NB)]) + '\n')
                            indx += 1
        if 'htstr' in reactor.solve:
            for i in range(reactor.solid.nhtstr):
                fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                    ['{0:12.5e} '.format(reactor.solid.htstr[i].temp[k]) for k in
                     range(reactor.solid.htstr[i].nr)]) + '\n')
                indx += 1
        if 'pointkinetics' in reactor.solve:
            ...
            # point kinetics cdnp
            fid[indx].write('{0:12.5e} '.format(time) + ''.join(
                ['{0:12.5e} '.format(reactor.core.cdnp[i]) for i in range(reactor.core.ndnp)]) + '\n')
            indx += 1
        if 'spatialkinetics' in reactor.solve:
            for i in range(reactor.core.niso):
                if reactor.core.iso[i].print_xs:
                    ...
                    for itemp in range(ntemp):
                        ...
                        fid[indx].write(' ' + 'igroup/sig0'.ljust(12) + ''.join(
                            ['{0:12.5e} '.format(reactor.core.iso[i].sig0[isig0]) for isig0 in range(nsig0)]) + '\n')
                        for ig in range(reactor.core.ng):
                            fid[indx].write(' ' + str(ig + 1).ljust(12) + ''.join(
                                ['{0:12.5e} '.format(reactor.core.iso[i].xs['tot'][ig][itemp][isig0]) for isig0 in
                                 range(nsig0)]) + '\n')
                    if sum(reactor.core.iso[i].xs['chi']) > 0:
                        for itemp in range(ntemp):
                            fid[indx].write(
                                'fission XS @' + '{0:12.5e} '.format(reactor.core.iso[i].temp[itemp]) + 'K \n')
                            fid[indx].write(' ' + 'igroup/sig0'.ljust(12) + ''.join(
                                ['{0:12.5e} '.format(reactor.core.iso[i].sig0[isig0]) for isig0 in
                                 range(nsig0)]) + '\n')
                            for ig in range(reactor.core.ng):
                                fid[indx].write(' ' + str(ig + 1).ljust(12) + ''.join(
                                    ['{0:12.5e} '.format(reactor.core.iso[i].xs['fis'][ig][itemp][isig0]) for isig0 in
                                     range(nsig0)]) + '\n')
                        for itemp in range(ntemp):
                            fid[indx].write('nubar @' + '{0:12.5e} '.format(reactor.core.iso[i].temp[itemp]) + 'K \n')
                            fid[indx].write(' ' + 'igroup'.ljust(12) + '\n')
                            for ig in range(reactor.core.ng):
                                fid[indx].write(' ' + str(ig + 1).ljust(12) + '{0:12.5e} '.format(
                                    reactor.core.iso[i].xs['nub'][ig][itemp]) + '\n')
                        fid[indx].write('fission spectrum\n')
                        fid[indx].write(' ' + 'igroup'.ljust(12) + 'chi'.ljust(12) + '\n')
                        for ig in range(reactor.core.ng):
                            fid[indx].write(' ' + str(ig + 1).ljust(12) + '{0:12.5e} '.format(
                                reactor.core.iso[i].xs['chi'][ig]) + '\n')
                    ...
                    for ig in range(reactor.core.ng):
                        fid[indx].write(' ' + str(ig + 1).ljust(12) + ''.join(
                            ['{0:12.5e} '.format(reactor.core.iso[i].xs['kerma'][ig][isig0]) for isig0 in
                             range(nsig0)]) + '\n')

                    for itemp in range(ntemp):
                        fid[indx].write(
                            'elastic scattering XS @' + '{0:12.5e} '.format(reactor.core.iso[i].temp[itemp]) + 'K \n')
                        fid[indx].write(' ' + 'from'.ljust(13) + 'to/sig0'.ljust(12) + ''.join(
                            ['{0:12.5e} '.format(reactor.core.iso[i].sig0[isig0]) for isig0 in range(nsig0)]) + '\n')
                        for s in reactor.core.iso[i].xs['elan'][0]:
                            fid[indx].write(' ' + str(s[0][0] + 1).ljust(13) + str(s[0][1] + 1).ljust(12) + ''.join(
                                ['{0:12.5e} '.format(s[1][isig0]) for isig0 in range(nsig0)]) + '\n')
                    ...
                    for s in reactor.core.iso[i].xs['ine']:
                        fid[indx].write(
                            ' ' + str(s[0][0] + 1).ljust(13) + str(s[0][1] + 1).ljust(12) + '{0:12.5e} '.format(
                                s[1]) + '\n')
            ...
            for i in range(reactor.core.nmix):
                if reactor.core.mix[i].print_xs:
                    ...
                    fid[indx].write(' ' + 'igroup'.ljust(13) + ''.join(
                        [str(reactor.core.mix[i].isoid[j]).ljust(13) for j in range(reactor.core.mix[i].niso)]) + '\n')
                    for ig in range(reactor.core.ng):
                        fid[indx].write(' ' + str(ig + 1).ljust(12) + ''.join(
                            ['{0:12.5e} '.format(reactor.core.mix[i].sig0[ig][j]) for j in
                             range(reactor.core.mix[i].niso)]) + '\n')
                    fid[indx].write(
                        'total XS, production XS, fission spectrum, in-group scattering XS, out-group scattering XS, n2n XS, kerma-factors\n')
                    fid[indx].write(' ' + 'igroup'.ljust(13) + 'sigt'.ljust(13) + 'nu*sigf'.ljust(13) + 'chi'.ljust(
                        13) + 'sigsi'.ljust(13) + 'sigso'.ljust(13) + 'sign2n'.ljust(13) + 'kerma'.ljust(13) + '\n')
                    for ig in range(reactor.core.ng):
                        sigso = 0
                        sigsi = 0
                        ...
                        fid[indx].write(
                            ' ' + str(ig + 1).ljust(12) + str('{0:12.5e} '.format(reactor.core.mix[i].sigt[ig])) + str(
                                '{0:12.5e} '.format(reactor.core.mix[i].sigp[ig])) + str(
                                '{0:12.5e} '.format(reactor.core.mix[i].chi[ig])) + str(
                                '{0:12.5e} '.format(sigsi)) + str('{0:12.5e} '.format(sigso)) + str(
                                '{0:12.5e} '.format(sign2n)) + str(
                                '{0:12.5e} '.format(reactor.core.mix[i].kerma[ig])) + '\n')
            ...
            if flag == 0: fid[indx].write(''.join(
                [(' ' + str(niter)).ljust(13) + '{0:12.5e} '.format(reactor.core.k[niter]) + '\n' for niter in
                 range(len(reactor.core.k))]))
            indx += 1
            # neutron flux
            if flag == 0:
                        ...
                        for iy in range(reactor.core.ny):
                            ...
                            if imix >= 0:
                                for ig in range(reactor.core.ng):
                                    ...
                                    fid[indx].write(
                                        '{0:12.5e} '.format(time) + ' ' + str(ig + 1).ljust(13) + str(iz).ljust(
                                            13) + str(ix).ljust(13) + str(iy).ljust(12) + '{0:12.5e} '.format(
                                            flux) + '\n')
            indx += 1
            # Adjoint flux
            if reactor.core.meth == 'AD':
                        ...
                        for ix in range(reactor.core.nx):
                            for iy in range(reactor.core.ny):
                                ...
                                if imix >= 0:
                                    for ig in range(reactor.core.ng):
                                        flux_a = sum(
                                            [reactor.core.flux_a[iz][ix][iy][it][ig] for it in range(reactor.core.nt)])
                                        fid[indx].write(
                                            '{0:12.5e} '.format(time) + ' ' + str(ig + 1).ljust(13) + str(iz).ljust(
                                                13) + str(ix).ljust(13) + str(iy).ljust(12) + '{0:12.5e} '.format(
                                                flux_a) + '\n')
                    ...
                    for ix in range(reactor.core.nx):
                        for iy in range(reactor.core.ny):
                            ...
                            if imix >= 0 and reactor.core.pow[iz][ix][iy] > 0:
                                fid[indx].write(
                                    '{0:12.5e} '.format(time) + ' ' + str(iz).ljust(13) + str(ix).ljust(13) + str(
                                        iy).ljust(12) + '{0:12.5e} '.format(reactor.core.pow[iz][ix][iy]) + '\n')
            indx += 1
            if flag == 0:
                for ix in range(reactor.core.nx):
                    for iy in range(reactor.core.ny):
                        if reactor.core.powxy[ix][iy] > 0:
                            fid[indx].write('{0:12.5e} '.format(time) + ' ' + str(ix).ljust(13) + str(iy).ljust(
                                12) + '{0:12.5e} '.format(reactor.core.powxy[ix][iy]) + '\n')
            indx += 1
